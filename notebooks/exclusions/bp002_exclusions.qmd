...
df_bp002_excl |>
  ...
  labs(x = NULL, y = "Exclusions from QOF indicator", colour = NULL) +
  ...
  ggplot2::geom_vline(
           ...
           size = 1) +
  ggplot2::geom_vline(
           xintercept = lubridate::as_date(c("2029-04-01", "2020-04-01", "2021-04-01")),
           linetype = "dotted",
           colour = "orange",
           size = 1) +
...
df_bp002_excl |>
  filter(group == "population") |>
  ggplot(aes(x = date, y = excl_denominator, colour = category)) +
  geom_point() +
  geom_line(alpha = .5) +
  facet_grid(vars(excl_rule), vars(group)) +
